...
const App = {
    ...
    setTransplant: async function(D_name, P_name, D_medical_id, P_medical_id, D_blood_type, P_blood_type, organ, Transplant_id) {
        const gas = await this.contractInstance.methods.setTransplant(
            D_name, P_name, D_medical_id, P_medical_id, D_blood_type, P_blood_type, organ, Transplant_id
        ).estimateGas({
            from: this.accounts[0]
        });
    
        await this.contractInstance.methods.setTransplant(
            D_name, P_name, D_medical_id, P_medical_id, D_blood_type, P_blood_type, organ, Transplant_id
        ).send({
            from: this.accounts[0], gas: Math.max(gas, MIN_GAS)
        });
    ...
    transplantMatch: async function() {
        ...
        console.log(this.contractInstance.options.jsonInterface);
        ...
        let matchedDonorIDs = []; // Maintain a list of matched donor IDs
        ...
        for (var i = 0; i < patientCount; i++) {
            ...
            for (var poi = 0; poi < patientorgans.length; poi++) {
                ...
                for (var j = 0; j < donorCount; j++) {
                    if (matchedDonorIDs.includes(donor[j].ID)) {
                        continue; // Skip this donor if already matched
                    }
                    ...
                    for (let doi = 0; doi < donor[j].organcount; doi++) {
                        ...
                        if (patientbloodtype == donor[j].bloodtype && patientorgans[poi] == donor[j].organs[doi]) {
                            ...
                            console.log("Matched: " + patientname + " " + patientorgans[poi] + "<->" + donor[j].name + " " + donor[j].organs[doi]);
                             ...
                             // Generate a unique Transplant ID
                        let transplantId = `${patientIDs[i]}-${donor[j].ID}-${patientorgans[poi]}`;

                        // Validate if transplant is already stored
                        let validateTransplant = await this.contractInstance.methods.validateTransplant(transplantId).call();
                        console.log("Transplant validation: ", validateTransplant);

                        // If transplant is not already stored, store it
                        if (!validateTransplant) {
                             this.setTransplant(
                                donor[j].name, patientname, donor[j].ID, patientIDs[i], donor[j].bloodtype, patientbloodtype, [patientorgans[poi]], transplantId
                            );

                            console.log("Transplant stored: ", transplantId);
                        } else {
                            console.log("Transplant already stored: ", transplantId);
                        }

                            match = [
                                { "Patient Name": patientname, "Patient Organ": patientorgans[poi], "Patient Medical ID": patientIDs[i], "": "↔️", "Donor Medical ID": donor[j].ID, "Donor Organ": donor[j].organs[doi], "Donor Name": donor[j].name },
                            ];
                            ...
                            matchedDonorIDs.push(donor[j].ID); // Add matched donor to the list
                            break;
                        }
                    }
